...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check if this is a checkout page visit, add to cart action, About Us page visit, or Instagram click
    ...
    const isInstagramClick = action === 'instagram_click';

    let message: string;
    if (isInstagramClick) {
      // Special notification for Instagram button click
      message = [
        '📸 <b>✨ INSTAGRAM BUTTON CLICKED ✨</b> 📸',
        '',
        '📱 <b>A user clicked to view @yomnoo on Instagram!</b> 📱',
        '',
        `🔗 <b>Page:</b> <a href="${url}">${url}</a>`,
        ...
        '',
        '📷 <b>User is showing high brand interest!</b> 📷'
      ].join('\n');
    } else if (isAddToCart && productTitle) {
      ...
      message = [
        ...
        `⏰ <b>Time:</b> <code>${time}</code>`,
        '',
        'ℹ️ <b>User is learning about Yomnoo!</b> ℹ️'
      ].join('\n');
    } else {
    ...
    try {
      await fetch(tgUrl, {
        ...
        body: JSON.stringify({
          ...
          // For high-intent actions, ensure sound notification is active
          ...((isCheckoutPage || isAddToCart || isAboutPage || isInstagramClick) && { disable_notification: false }),
        }),
      });
    } catch (e) {
